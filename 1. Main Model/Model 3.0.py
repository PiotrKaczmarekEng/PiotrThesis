...
data_file = os.getcwd() 
data_file = os.path.dirname(os.path.realpath('__file__')) + '\Main Model\Parameter Data.xlsx' # Locate file
...
#     sensitivity_file_path = "Try 15 (no nh3 reconv)/"
...
for iteration_j in range(0,4):        # range(2) to study optimal location of j1 and j2, range(2,4) for j3 and j4, range(0,4) for all j in J
    ...
    # Tycho's numbers
    lftm = 25 # Expected lifetime of equipment [years]
    a = (InterestRate*(1+InterestRate)**lftm) / ((1+InterestRate)**lftm - 1) # Amortization factor
    
    ThroughputConv = 100000/365     # Daily throughput of a conversion device [tonNH3/day]
    ThroughputReconv = 1200         # Daily throughput of a reconversion device [tonNH3/day]
    TotalDirectCostConv = 80        # Costs of conversion device [MEur]
    TotalDirectCostReconv = 63      # Costs of reconversion device [MEur]
    FactorConv = (TotalDirectCostConv/TotalDirectCostReconv)*(ThroughputReconv/ThroughputConv)  # Conversion - Reconversion Factor
    
    # Reconversion costs and capacity
    CAPEX_1 = 94500000              # [Eur]
    OPEX_1 = 1600000                # [Euro/y]
    Capacity_reconv = 78840         # [tonH2/y]
    Capacity_used_device = 10000
    
    # Prepare Reconversion cost array
    ...
    def rel_con_array(SY, NS, TS, ConNH3Co):
        ...
        Inputs - SY: StartingYear
                  NS: NSteps
                  TS: Time Step
                  ConNH3Co: Full NH3 conversion costs array
        Outputs - NH3 conversion costs array of relevant format
        '''
    ...
    def rel_rec_array(SY, NS, TS, RecNH3Co):
        ...
        Inputs - SY: StartingYear
                  NS: NSteps
                  TS: Time Step
                  RecNH3Co: Full NH3 reconversion costs array
        Outputs - NH3 reconversion costs array of relevant format
        '''
    ...
    for vert in range(size):
        ...
        for loc in loc_matrix[vert]:
            ...
            Creconvammonia = np.zeros(Nsteps) # No reconversion situation
            ...
            for l in L: 
                ...
                # print("Objective Value (scaled by DYR): ", model.ObjVal)
                # print("Objective Value rescaled to a year: ", model.ObjVal*(1/DataYearRatio))
