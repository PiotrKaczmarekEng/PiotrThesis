...
# Energy Medium (0: Ammonia, 1: Liquid Hydrogen)
E = 0
...
def LCOT_Ammonia(distance):
    ...
    # print('Estimated LCOT NH3: ',LCOT_prediction, '[Eur/kg]')
    # Assuming no learning rate
    LCOT_pred_array = np.zeros(Nsteps)
    CurrentYear = Startyear
    for l in range(Nsteps):
        if CurrentYear <= 2030:
            LCOT_pred_array[l] = LCOT_prediction
            CurrentYear += timestep
        elif CurrentYear > 2030:
            LCOT_pred_array[l] = LCOT_prediction
            CurrentYear += timestep
        # print(LCOT_pred_array[l])
    return LCOT_pred_array

def LCOT_H2(distance):
    m=1/1600
    c = 0
    LCOT_prediction = m*distance+c
    # print('Estimated LCOT LH2: ',LCOT_prediction, '[Eur/kg]')
    # Assuming learning rate of 0.9 until 2030 and 0.5 until 2050
    LCOT_pred_array = np.zeros(Nsteps)
    CurrentYear = Startyear
    for l in range(Nsteps):
        if CurrentYear <= 2030:
            LCOT_pred_array[l] = LCOT_prediction*0.9**(l+1)
            CurrentYear += timestep
        elif CurrentYear > 2030:
            LCOT_pred_array[l] = LCOT_prediction*0.5**(l+1)
            CurrentYear += timestep
        # print(LCOT_pred_array[l])
    return LCOT_pred_array

#### Input params for pipeline transport
...
#     # CAPEX row
#         CostPipeYear[1][i] = CAPEX_pipe*dist*1000000*25 # [USD]
#     # OPEX row
#         CostPipeYear[2][i] = (OPEX_pipe*0.01*CostPipeYear[1][i]) * DF #[USD/yr]
#     # Denominator row
#         CostPipeYear[3][i] = EnergyTransported / DF # [GWh/yr]
...
# LCOHT = LCOET_func(distancesea)*gamma/1000000 # USD/tonH2


# Cpipeline = Cpipe


# Energy medium set J (1 ammonia, 2 LH2)
J = [1, 2]

# Information about settings for run
if modesetting==0:
    print('Transport Mode set to: Shipping')
elif modesetting==1:
    print('Transport Mode set to: Pipeline')
    
if E==0:
    print('Energy Medium set to: NH3')
elif E==1:
    print('Energy Medium set to: LH2')

def func_TC(location, mode):
    
    location = [location[1],location[0]]
    distancesea = distanceseafactor*geopy.distance.geodesic(location, coords_port).km
    Xkmsea = Xtransport*distancesea
    Cpipeline = np.array([LCOT_Ammonia(distancesea), LCOT_H2(distancesea)])
    ...
    if mode==0:
        ...
        TC = []
        for j in J:
            TCyear = Cpipeline[j-1]*Xtransport*1000 # [Euro] (=[Euro/kg]*[ton]*[kg/ton])
            TC.append(TCyear)
            
    return TC

TC = func_TC(area,modesetting)
...
size = 2  # This will create a 3x3 matrix
...
for vert in range(size):
    ...
    for loc in loc_matrix[vert]:
        ...
        # Calculate TC
        TC = func_TC(loc,modesetting)
        print('Currently in location: ', str(loc))
        ...
        # Battery Cost
        BatCost = 1 #Set to 1 euro for now
        ...
        MaxDischarge = 100 #[W/hr] 
        # Max energy capacity in battery
        MaxBatteryCapacity = 1000 #[W]
        ...
        # Battery
        b = model.addVar (lb = 0, vtype = GRB.INTEGER, name = 'b' )
        
        # Charge
        for t in T:
            charge[t] = model.addVar (lb = 0, vtype = GRB.CONTINUOUS, name = 'charge' + str(t) + ']' )
            discharge[t] = model.addVar (lb = 0, vtype = GRB.CONTINUOUS, name = 'discharge' + str(t) + ']' )
        
        #EnergyStored
        EnergyStored = model.addVar (lb = 0, vtype = GRB.CONTINUOUS, name = 'EnergyStored' )
        ...
        for t in T:
            ...
            model.addConstr(PU[t] + charge[t] == alpha[E]*PG[t] + discharge[t]) # Changed for battery
            # model.addConstr(PU[t] <= beta*gamma*x[3] )     #Removed for battery
            model.addConstr(charge[t] == PG[t] - beta*gamma*x[3]) # Added for battery
        ...
        # Added for battery below
        model.addConstr(EnergyStored == quicksum(charge[t] for t in T))
        model.addConstr(EnergyStored >= quicksum(discharge[t] for t in T))
        ...
        for l in L: 
            ...
            exceldf[timestep*l+Startyear] = [demand*(1/DataYearRatio),demandlocation,timestep*l+Startyear, loc, transferport, model.ObjVal*(1/DataYearRatio), model.ObjVal/demand/1000,x[1].X, x[2].X, x[3].X, x[4].x, y[1].X, W[0][E], W[1][E], transportmedium, y[2].X,distancesea,distanceland]
            dict_of_df[loc][timestep*l+Startyear] = [demand*(1/DataYearRatio),demandlocation,timestep*l+Startyear, loc, transferport, model.ObjVal*(1/DataYearRatio), model.ObjVal/demand/1000,x[1].X, x[2].X, x[3].X, x[4].x, y[1].X, W[0][E], W[1][E], transportmedium, y[2].X,distancesea,distanceland]
